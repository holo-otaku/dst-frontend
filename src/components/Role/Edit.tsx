...
import { Stack, Button, Form, InputGroup, Row, Col, FormGroup, FormCheck } from "react-bootstrap";
import useAxios from "axios-hooks";
import { ScaleLoader } from "react-spinners";
import { useNavigate, useParams } from "react-router-dom";
import Backdrop from "../Backdrop/Backdrop";
import { RoleResponse, PermissionResponse, CreateRolePayload, PermissionData, RoleDetailResponse } from './Interfaces';
...
export const Edit = () => {
    ...
    const [{ loading: editLoading }, editRole] =
    ...
    const [{ data: permissionResponse, loading: permissionLoading }, refetchPermission] =
    ...
    useEffect(() => {
        if (detailResponse?.data) {
            ...
            setRolePermissionIds(detailResponse.data.permissions.map((perm) => perm.id));
        }
    }, [detailResponse]);
    ...
    return (
        <Stack gap={2}>
            ...
            <Stack direction="horizontal" gap={3} className="mt-3">
                ...
                <Button variant="primary" disabled={!rolename || rolePermissionIds.length === 0} onClick={handleSubmit}>
                    更新
                </Button>
